...
export default function AdminDashboard() {
        ...
        <div className="bg-white p-4 rounded shadow" >
            <h2 className="text-gray-500" > Total Leads </h2>
                < p className = "text-3xl font-bold" > { data.totalLeads } </p>
                    ...
                    < div className = "bg-white p-4 rounded shadow" >
                        <h2 className="text-gray-500" > Active Agents </h2>
                            < p className = "text-3xl font-bold" > { data.agentPerformance.length } </p>
                                </div>
                                < div className = "bg-white p-4 rounded shadow" >
                                    <h2 className="text-gray-500" > Overall Completion </h2>
                                        < p className = "text-3xl font-bold" >
                                            { data.agentPerformance.reduce((acc: number, a: any) => acc + a.completed, 0) } / { data.totalLeads }
                                            </p>
                                            </div>
                                            </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8" >
        <div className="bg-white p-4 rounded shadow" >
            <h2 className="text-xl font-semibold mb-2" > Leads by Status </h2>
                < ResponsiveContainer width = "100%" height = { 300} >
                    ...
                    <Pie data={ data.statusDistribution } dataKey = "count" nameKey = "_id" cx = "50%" cy = "50%" outerRadius = { 80} label >
                    {
                        ...
                        < div className = "bg-white p-4 rounded shadow" >
                            <h2 className="text-xl font-semibold mb-2" > Leads by Priority </h2>
                                < ResponsiveContainer width = "100%" height = { 300} >
        ...
        <h2 className="text-xl font-semibold mb-2" > Agent Performance </h2>
            < table className = "min-w-full border" >
                ...
                <th className="border p-2" > Agent </th>
                    < th className = "border p-2" > Assigned Leads </th>
                                    ...
                                    <tbody>
    {
        ...
        < td className = "border p-2" > { agent.totalAssigned } </td>
        < td className = "border p-2" > { agent.completed } </td>
        < td className = "border p-2" > { agent.inProgress } </td>
        < td className = "border p-2" > { agent.completionRate } % </td>
        </tr>
        ))
    }
    </tbody>
